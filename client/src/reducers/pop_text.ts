  ...
  (builder) => {
    builder
      .addCase("DOUBLES", (state, action) => {
        state.popText.push({ text: "Doubles!", color: "cyan", id: state.popTextId++ });
      })
      .addCase("win", (state, action) => {
        state.popText.push({ text: "{winner} wins!", color: "lime", id: state.popTextId++ });
      })
      ...
      .addCase("update", (state, action: UpdateMsg) => {
        if (action.reset) {
          state.popText.push({ text: "Reset!", color: "red", id: state.popTextId++ });
        }
        else {
      ...
      .addCase("update_turn", (state, action: UpdateTurnMsg) => {
        if (state.has69) {
          state.popText.push({ text: "Nice.", color: "yellow", id: state.popTextId++ });
          state.has69 = false;
        }
      })
